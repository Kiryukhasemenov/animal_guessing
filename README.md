...
3. The [code]() and the [intents]() are organized as follows:
 * after `start` intent, the agent imports the [`animals.csv`]() file with the information about the animals and picks a random animal (functions `making_dataset` and `taking_random_animal`). Until the animal is guessed correctly, it is fixed as a global variable.
...
| File | description |
| --- | --- |
| [animal_guessing.py]() | the main code with the dialog system |
| [animal_guessing.zip]() | the backup with the intents, entities and other elements of the Dialogflow framework |
| [animals.csv]() | the dataset with the animals and their parameters |
